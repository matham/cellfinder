...
class DataAugmentation:
    ...
    _intensity_range: float | tuple[float, float] | None = None
    ...
    def update_parameters(self) -> bool:
        ...
        for aug in ["rotate", "translate", "scale"]:
            if getattr(self, f"_{aug}_trans") is not None and random_bool(likelihood=prob):
    ...
    def needs_isotropic(self) -> bool:
        return bool({"rotate", "translate", "scale"} & self._augmentations_to_do)
    ...
    def apply_intensity(self, data: torch.Tensor) -> torch.Tensor:
        ...
        i_range = self._intensity_range
        if isinstance(i_range, Sequence):
            s, e = i_range
