...
def filter_for_peaks(
    ...
    filtered_planes = F.conv2d(filtered_planes, x_kernel, padding="same")
    filtered_planes = F.conv2d(filtered_planes, y_kernel, padding="same")

    # it is 2d
    lap_kernel = normalize_kernel2d(lap_kernel)
...
class PeakEnchancer:
    ...
    def __init__(
        ...
        self.lap_kernel = get_laplacian_kernel2d(3, device=device, dtype=dtype)

    def enhance_peaks(self, planes: torch.Tensor) -> torch.Tensor:
        filtered_planes = filter_for_peaks(
            planes, self.bin_kernel, self.gauss_kernel, self.lap_kernel
        )
