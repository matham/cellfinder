...
class ThresholdFilter3D:
    ...
    def __init__(
        ...
        self.side_data: list[torch.Tensor] = []
    ...
    def append(
        ...
        side_data: torch.Tensor | None = None,
    ) -> None:
        """
        ...
        if (
            side_data is not None
            and data_planes.shape[0] != side_data.shape[0]
        ):
            raise ValueError(
        ...
        if side_data is not None:
            for i in range(side_data.shape[0]):
                self.side_data.append(side_data[i, ...])
    ...
    def get_processed_planes(
        ...
        return (
            self.marked_planes[:num_processed],
            self.side_data[:num_processed],
        )
